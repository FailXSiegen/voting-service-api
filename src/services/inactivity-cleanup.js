...
export function startInactivityCleanup(interval = 60000) {
  ...
  console.info(
    `[Inactivity Cleanup] Service started, checking every ${interval / 1000} seconds, timeout set to ${activeTimeout} seconds`
  );
  ...
  if (LOAD_TEST_MODE) {
    console.info(
      '[Inactivity Cleanup] ⚠️ LOAD-TEST-MODUS: Verlängerter Inaktivitäts-Timeout aktiv (2 Stunden)'
    );
  }
    ...
    try {
      ...
      if (LOAD_TEST_MODE) {
        ...
        if (Array.isArray(loadTestEvents) && loadTestEvents.length > 0) {
          ...
          console.info(
            `[Inactivity Cleanup] Load-Test-Events gefunden, IDs: ${loadEventIds.join(', ')}. Diese werden vom Cleanup ausgeschlossen.`
          );
      ...
      if (LOAD_TEST_MODE && process.env.DISABLE_CLEANUP_IN_LOADTEST === '1') {
        console.info(
          `[Inactivity Cleanup] Load-Test-Modus mit DISABLE_CLEANUP_IN_LOADTEST aktiv, überspringe Cleanup vollständig`
        );
        return;
      }
      ...
      if (LOAD_TEST_MODE) {
        console.info(
          `[Inactivity Cleanup] Load-Test-Modus aktiv. Timeout verlängert auf ${LOAD_TEST_INACTIVITY_TIMEOUT} Sekunden.`
        );
      }
      ...
      if (result && result.affectedRows > 0) {
        console.info(
          `[Inactivity Cleanup] ${result.affectedRows} inaktive Benutzer als offline markiert`
        );
      }

      if (activeEventIds.length > 0) {
        console.info(
          `[Inactivity Cleanup] ${activeEventIds.length} Events mit aktiven Abstimmungen oder Load-Tests ausgeschlossen`
        );
      }
    } catch (error) {
