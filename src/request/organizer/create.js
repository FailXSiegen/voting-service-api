...
export default async function createOrganizer (req, res) {
  ...
  try {
    ...
    if (await findOneByUsername(organizerArguments.username) !== null) {
      throw new Error('organizer with the following username already exists: ' + organizerArguments.username)
    }
    ...
    if (!organizerId) {
      throw new Error('Could not create organizer with the following username: ' + organizerArguments.username)
    }
