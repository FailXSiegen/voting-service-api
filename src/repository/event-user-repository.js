...
export async function findOneByUsernameAndEventId (username, eventId) {
  const result = await query('SELECT event_user.* FROM event_user INNER JOIN event ON event_user.event_id = event.id WHERE event_user.username = ? AND event.id = ? AND event.deleted = 0', [username, eventId])
  return Array.isArray(result) ? result[0] || null : null
}
...
export async function findOnlineEventUserByEventId (eventId) {
  return await query('SELECT * FROM event_user WHERE event_id = ? AND verified = 1 AND online = 1 AND allow_to_vote = 1 ORDER BY public_name ASC', [eventId])
}
