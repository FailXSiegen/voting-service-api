...
poll_user_id = ?,
create_datetime = ?`,
      [input.pollResultId, input.possibleAnswerId, input.answerContent, pollUserId.id, getCurrentUnixTimeStamp()])
  } else {
    await query(
      `INSERT INTO poll_answer SET poll_result_id = ?,
poll_possible_answer_id = ?,
answer_content = ?,
create_datetime = ?`,
      [input.pollResultId, input.possibleAnswerId, input.answerContent, getCurrentUnixTimeStamp()])
  }
}
