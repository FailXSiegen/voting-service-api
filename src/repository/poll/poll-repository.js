import { insert, update as updateQuery, remove as removeQuery, query } from './../../lib/database';
...
export async function findPollsByEventId(eventId) {
  return await query('SELECT poll.* FROM poll WHERE poll.event_id = ?', [eventId]);
}

async function deltePollReferences(pollId) {
  const pollResult = await findOnePollResultByPollId(pollId);
  if (pollResult) {
    await query('DELETE FROM poll_user_voted WHERE poll_result_id = ?', [pollResult.id]);
