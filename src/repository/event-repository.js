...
export async function findById(id) {
  const result = await query('SELECT * FROM event WHERE id = ? AND deleted = 0', [id]);
...
export async function findByOrganizer(organizerId) {
  return await query('SELECT * FROM event WHERE organizer_id = ?  AND deleted = 0', [organizerId]);
}
...
export async function getMultivoteType(eventId) {
  const result = await query('SELECT event.multivote_type FROM event WHERE event.id = ?', [
    eventId,
  ]);
...
export async function isAsyncEventStarted(eventId) {
  const result = await query('SELECT async, scheduled_datetime FROM event WHERE id = ?', [eventId]);
...
export async function update(input) {
  ...
  const event = await query('SELECT * FROM event WHERE id = ? AND deleted = 0', [input.id]);
  ...
  if (
    ...
    (eventData.originalOrganizerId &&
      parseInt(organizerId) === parseInt(eventData.originalOrganizerId))
  ) {
