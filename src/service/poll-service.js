...
export async function existsPollUserVoted(pollResultId, eventUserId, multiVote) {
  try {
    console.log(
      `[DEBUG:EXISTS_POLL_USER_VOTED] Starting check for user ${eventUserId}, pollResult ${pollResultId}, multiVote ${multiVote}`
    );
    ...
    if (!eventUser.verified || !eventUser.allowToVote) {
      console.warn(
        `[WARN] existsPollUserVoted: Benutzer ${eventUserId} ist nicht verifiziert (${eventUser.verified}) oder darf nicht abstimmen (${eventUser.allowToVote})`
      );
      return false;
    }
    ...
    if (!userExists) {
      console.log(
        `[DEBUG:EXISTS_POLL_USER_VOTED] User ${eventUserId} hat noch keinen poll_user_voted Eintrag - erstelle neuen`
      );
      ...
      console.log(
        `[DEBUG:EXISTS_POLL_USER_VOTED] Neuer Eintrag für User ${eventUserId} mit voteCycle=0, maxVotes=${maxVotes}, multiVote=${multiVote}`
      );

      // Repository-Funktion verwenden
      await createPollUserVoted(pollResultId, eventUserId, voteCycle);
      console.log(
        `[DEBUG:EXISTS_POLL_USER_VOTED] poll_user_voted Eintrag erstellt für User ${eventUserId}`
      );
      ...
      if (multiVote) {
        ...
        if (maxVotes === 1) {
          ...
          console.log(
            `[DEBUG:MULTIVOTE] User ${eventUserId} hat nur 1 Stimme - erlaube erste Abstimmung`
          );
          return true;
        }
        ...
        if (voteCycle === maxVotes) {
          ...
          console.log(
            `[DEBUG:MULTIVOTE] User ${eventUserId} beginnt MultiVote (voteCycle=${voteCycle}, maxVotes=${maxVotes})`
          );
          return true;
        }
      }
      ...
      console.log(
        `[DEBUG:EXISTS_POLL_USER_VOTED] Returning TRUE for new user ${eventUserId} (single vote mode)`
      );
      return true; // Bei normalem Modus: Erste Stimme erlauben
    }

    console.log(
      `[DEBUG:EXISTS_POLL_USER_VOTED] User ${eventUserId} hat bereits poll_user_voted Eintrag - prüfe allowToCreateNewVote`
    );
    ...
    console.log(
      `[DEBUG:EXISTS_POLL_USER_VOTED] allowToCreateNewVote result for user ${eventUserId}:`,
      result
    );
    return result;
  } catch (error) {
